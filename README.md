...
- Android 전자동의서 고도화(2022.10 ~ 2023.04)

   1. 웹 뷰에서 동작하는 JavaScript 소스 리팩토링 진행

      * JavaScipt 소스코드 중 영향도 분석을 통하여 사용하지 않는 코드 삭제 및 축약 변경

      * 적용이후 코드 분석 및 개발 시간 약 20% 단축

   2. 솔루션 별 SDK 소스를 단일 프로젝트로 통합

      * 안드로이드 스튜디오의 IDE에서 Build Variant 기능과 Gradle의 Product Flavor를 통하여 빌드하고자 하는 앱 선택 후 빌드가 가능하도록 개발

      * 소스 통합 후 앱 별 빌드 후 배포시간 평균 2배 이상 단축

   3. 클린아키텍쳐 구현, Kotlin 변환

      * 소스 통합 후 UI레이어, Domain레이어, Data레이어로 분할

      * 새로 작성하는 코드는 Kotlin으로 작성

- Android 채팅 앱 개발(2022.07 ~ 2022.08)
...
- Android Webview 기능 개발(2022.06 ~ 2022.09)

   * 역할 : web api 사용하여 Webview 추가 기능 개발

      * 텍스트 박스 위치 조절 기능, 그림 삽입 기능, 브러쉬 위치 조절 기능 구현
...
- 병원 고객앱 SNS 연동 시스템 개발(2021.08 ~ 2021.09)
   ...
   * 역할 : 병원 홈페이지, 앱의 회원가입을 간편하게 하기 위한 SNS로그인 도입

      * SNS(NAVER, KAKAO) Api 등록 후 토큰을 서버에 저장하고, 로그인 시 토큰을 대조하는 방식으로 개발

- Android 영상 플레이어 앱 개발(2021.06 ~ 2021.09)

   * 역할 : 의료진이 병동 태블릿을 활용하여 환자에게 영상을 시청하게하는 목적의 앱 개발

      * 원내 내부망 스트리밍 서버의 RestApi 를 활용하여 영상 스트리밍 서비스를 제공하는 App 개발

- 교직원 전용 앱 기능 개발(2020.12.01 ~ 2021.03.01)
   
   * 역할 : 의료진이 병원 회진 시 회진문자 자동 전송 서비스 개발
  
     * 원내 HIS SMS 메세지 서버에 데이터를 연결하는 Api 개발

     * 회진문자 시스템 사용 시 포상제도로 인해 전체 의사의 80%가 사용
...
<img src="https://img.shields.io/badge/Objective--C-147EFB?style=flat-square&logo=Objective--C&logoColor=white"/></a> &nbsp
</p>
